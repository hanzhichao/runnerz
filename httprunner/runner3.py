...
def parameters_to_ddt(parameters):
    # for line in parameters:
    line = parameters[0]
    keys, data = tuple(line.items())[0]
    keys = keys.split('-')
    return dict(keys=keys, data=data)

env = jinja2.Environment()
env.filters['ddt'] = parameters_to_ddt
...
class TestApi(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.context = {{config.variables|default({})}}
        for key, value in config_request.items():
            setattr(cls.session, key, value)
            
    {% for test in data[1:]%}{% set test=test.test %}
    {% if test.skip %}@unittest.skip('skip=True'){% endif %}
    {% if test.parameters %}{% set line=test.parameters[0] %}{% set keys=line.keys()|list %}{% set keys=keys[0].split("-")%}{% set data=line.values()|list%}{% set data=data[0] %}@ddt.data(*{{data}}){% endif %}
    def test_api_{{loop.index}}(self{% if test.parameters %}, *keys{% endif %}):
        """{{test.name}}"""
        test = {{test}}
        test = parse_dollar(self.context, test)
        send_request(self.context, self.base_url, self.session, test.get('request'))
        do_extract(self.context, test.get('extract', []))
        do_validate(self.context, test.get('validate', []))
    {% endfor %}
    '''
    # print(tmp)
    rendered = jinja2.Template(tmp).render(config=config, data=data)
    # print(rendered)
    exec(rendered, globals(), dict(config=config))
    test_class = locals().get('TestApi')
    from pprint import pprint
    pprint(globals())
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(test_class)
    return suite
    # exec(tmp, globals(), dict(config=config))
    # print(vars())
