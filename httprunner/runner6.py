...
def do_test(self, test: dict) -> None:
    ...
    # self.context['request'] = {key: value for key, value in request.__dict__.items()}
...
# def parse_result(result: unittest.TestResult) -> dict:
#     stat = dict(
...
#     )
#     stat['successes'] = stat['testsRun'] - stat['skipped'] - stat['failures'] - stat['errors'] - stat['expectedFailures'] - stat['unexpectedSuccesses']
#     return stat
...
def run(suite: unittest.TestSuite) -> unittest.TestResult:
    ...
    # for test, exec_info in result.failures:
